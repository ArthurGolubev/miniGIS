...
from server.calculation.classification.unsupervision.BisectingKMeans import BisectingKMean
from server.calculation.classification.unsupervision.GaussianMixture import GaussianMixture


router = APIRouter(
    prefix='/unsupervision',
    )
...
async def classify_mean_shift(options: KMeanOptions, user: User1 = Depends(get_current_user)):
