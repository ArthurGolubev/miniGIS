...
export const DetailVec = () => {
    ...
    const addAttrToLayerHandler = () => {
        ...
        Object.keys( layersSub[selectedVecLaySub].layer._layers).map((key: string) => layersSub[selectedVecLaySub].layer._layers[key].feature.properties[`${attrName}_${attrType}`] = '' )
    ...
    return <div>
                                            ...
                                            return <li key={propName} className="mb-2">
                                                {
                                                    (layersSub[selectedVecLaySub] as Shape).properties[propName].fieldType == 'L' ? (
                                                        <div className="form-check">
                                                            ...
                                                            type="checkbox"
                                                            value={(layersSub[selectedVecLaySub] as Shape).layer._layers[state.editPropShapeID].feature.properties[propName] ?? ""}
                                                        ...
                                                        <div className='input-group input-group-sm'>
                                                            ...
                                                            value={layersSub[selectedVecLaySub].layer._layers[state.editPropShapeID].feature.properties[propName] ?? ""}
                                        ...
                                        <th className='text-center'>
                                            {
                                                state.editPropShapeID != geomId ? (
                                                    ...
                                                    onClick={() => setState({...state, editPropShapeID: geomId})}></i>
                                                ) : (
                                                    <button
                                                    onClick={()=>setState({...state, editPropShapeID: undefined})}
                                                    className='btn btn-sm btn-close' type='button'></button>
                                                )
                                            }
                                        </th>
        ...
        {/* 
        TODO
            1. Сделать логику отправки слоя с формами и атрибутами на сервер
            2. Сделать форму "Название.shp"
        */}
